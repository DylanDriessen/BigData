...
nameSetUnique <- unique(dataSet2[dataSet2$entity=='John Powell',])
#View(nameSet)
...
wordcloud(frequencyDataSetUnique$entity.y, frequencyDataSetUnique$freq, min.freq=2,scale=c(8, .5),
...
nameSet <- (dataSet2[dataSet2$entity=='John Powell',])
View(nameSet)
...
wordcloud(frequencyDataSet2$entity.y, frequencyDataSet2$freq, min.freq=2,scale=c(8, .5),
          max.words=Inf, random.order=FALSE, rot.per=.15,
          colors = pal)
dev.off()
